...
void User::wrtTxt(){
    ...
    outTxt<<"Number of Surveys Taken: "; 
    outTxt<< (surveys/100) << (surveys/10%10) << (surveys%10) <<endl; 
...
void User::wrtBin(){
    ...
    // Write surveys to binary file
    outBin.write(reinterpret_cast<char *>(&surveys) , sizeof(int)); 
...
void User::reWrtTxt(long begnFile){
    ...
    long cursor = (begnFile <= 0) ? 0 : (begnFile+charCount); // surveys is a int which is 4 bits
    ...
    outTxt<<"surveys: "; 
    outTxt<< (getHiScore()/100) << (getHiScore()/10%10) << (getHiScore()%10) <<endl<<endl;
    outTxt.close(); // close file    
}
...
void User::reWrtBin(long begnFile){
    ...
    // Rewrite surveys in binary file     
    outBin.write(reinterpret_cast<char *>(&surveys) , sizeof(int)); 
...
//******************************************************************************
//            Checks if current score is greater than surveys 
//******************************************************************************

bool User::isHiScore(int score){
    
    if((score > 999 || score < 0)){ return false; }
    // surveys is formatted for max of 3 digits 
    
    else if(score > surveys ) { return true; }
...
bool User::isEmail(){
    ...
    const unsigned short MAX_ATTEMPTS = 4; // Limits how many surveys user can re-input
    unsigned short count = 0;  
    string em = "";
